...
def write_excel(start, end, sentence2id, id2sentence, mapping):
    ...
    filename = "RO-STS_{}-{}.xlsx".format(start, end)
    ...
    with open("file.json", "r", encoding="utf8") as f:
        ...
        for l in mls:
            ...
            if "annotations" in js:
                if len(js["annotations"])>0:
                    trans[js["text"]] = js["annotations"][0]["text"]
    ...
    for index in range(start, end):
        tra = ""
        if sentence2id[index] in trans:
            tra = trans[sentence2id[index]]
